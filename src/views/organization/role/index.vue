  ...
  <div class="app-container">
    ...
    >
      ...
      >
        ...
        >
          <el-row
            v-for="(v,name) in grouppedResources"
            :key="name"
          >
            <el-row style="padding-top:15px;">
              <el-tag>{{ name.toUpperCase() }}</el-tag>
            </el-row>
            <span
              v-for="(resource,index) in v"
              :key="index"
            >
              <el-checkbox
                :key="resource.id"
  ...
  methods: {
    ...
    handleAuth(id) {
      ...
      queryAllResource().then(response => {
        ...
        this.grouppedResources = response.data.reduce((p, c, i) => {
          const group = p[c.type] || []
          group.push(c)
          p[c.type] = group
          return p
        }, {})
